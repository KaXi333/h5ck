<template>
  <div class="navbar__wrapper navbar__wrapper-transform">
   <div class="ui text container navbar__content" style="width:1000px !important;">
    ...
    <!--  <div class="navbar__item m--computer-only">
     ...
     <div class="widget__dropdown m--computer-only widget__dropdown__left">
      ...
      <div class="widget__dropdown__list " :class="{'open': isShowGuide2}">
       <div v-for="(item,index) in detailPages" :key="item.id" class="widget__dropdown__item " @click="goDetailPageFn(index)">
...
export default {
  ...
  methods:{
    ...
    // 添加锚点滚动
    goSteps(index){
      if(this.$store.state.isSteps){
        this.$router.push('/')
        var that=this
        setTimeout(function (){
            that.$root.$emit('scrollFn',index);
        }, 500);
      }else{
